...
右键手势默认启用。按住鼠标右键并拖动，或按住右键再按另一个鼠标键，会触发配置文件中的快捷键。按住右键滚动滚轮会打开窗口切换器，滚轮向下前进、向上反向，松开右键确认选择。连续三击右键会进入隐私黑屏并隐藏鼠标指针；再次连续三击右键恢复。新 app 会在第一次启动时从下面这些旧配置里优先迁移第一个可用文件：
...
- 输入监控：核心权限，用于监听右键鼠标手势、右键滚轮切换、鼠标组合键、连续三击右键、双击 Option 和 Option 释放事件。
