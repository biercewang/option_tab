...
右键手势默认启用。按住鼠标右键并拖动，或按住右键再按另一个鼠标键，会触发配置文件中的快捷键。新 app 会在第一次启动时从下面这些旧配置里优先迁移第一个可用文件：
...
- 输入监控：核心权限，用于监听右键鼠标手势、鼠标组合键、双击 Option 和 Option 释放事件。
- 屏幕录制：可选权限，只用于生成本地窗口缩略图；默认关闭缩略图以避免请求。
- 自动化：条件权限，只在某些右键手势需要通过 System Events 触发全局快捷键时由 macOS 弹窗请求。
